...
(r[k].push(e),r[e].push(k)),a.history&&d.history.pushState(a.title,a.title,a.link));d.document.title=a.title;k=a.link}}function z(b,a){for(var c={},e={},f=[],d=q.length,g=0;g<q.length;g++)f.push(q[g]),e[q[g]]=d;for(var h=e[b]=0;0!==f.length;){h++;var n=f.sort(function(a,b){return e[a]-e[b]})[0];if(n===a){for(f=[];a;)f.push(a),a=c[a];return f.reverse()}for(var k=r[n],g=0;g<k.length;g++){var l=k[g];e[l]===d&&(e[l]=h,c[l]=n)}f.shift()}return null}function w(b){return function(a){t[b]||(t[b]=[]);t[b].push(a)}}
...
(function(d){function m(d){l=!1;var h=document.querySelector('#todo-list input[type="text"]');h?(h.focus(),h.value=h.value,h.addEventListener("blur",function(d){l||(h.value.trim()?refreshAjax.navigate({action:"edit",Title:h.value},h.form):refreshAjax.navigate({action:"delete"},h.form))}),h.addEventListener("keyup",function(d){l||27!==d.keyCode||refreshAjax.navigate({action:"edit",cancel:!0},h.form)})):"todo-form"===d.target.id&&document.getElementById("new-todo").focus()}if(d.refreshAjax){var l=!1;
refreshAjax.navigating(function(d,h){l=d.data&&"edit"===d.data.action});refreshAjax.updating(function(d,h){if("todo-form"===d.target.id){var g=0,m;for(m in h.panels){var p=Number(m.slice(4));p&&g<p&&(g=p)}g&&!document.getElementById("todo"+g)&&document.getElementById("todo-list").insertAdjacentHTML("beforeend",'<li><span id="todo'+g+'" /></li>')}d.data&&"edit"===d.data.action&&!l&&(h.panels=null)});refreshAjax.updated(function(d,h){if(d.target.getAttribute){var g=d.target.getAttribute("data-todo");
g&&!h.panels["todo"+g]&&(g=document.getElementById("todo"+g))&&(g=g.parentNode,g.parentNode.removeChild(g))}m(d)});m({target:{id:"todo-form"}})}})(window);